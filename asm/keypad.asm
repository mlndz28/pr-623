...
TT_MT:		jsr MUX_TECLADO				; mux_teclado();					(1)
			...
			brset Banderas,$02,PUSHED	; else if(TCL_LEIDA){pushed()}		(3)
			movb Tecla, Tecla_IN		; else{ Tecla_IN = Tecla;			(2)
			bset Banderas, $02			;   TCL_LEIDA = true;
			movb #100, Cont_Reb			; 	Cont_Reb = 100; // in ms
			rts							;   return;
										; } 
NOT_PUSHED: brset Banderas,$01,RELEASED	; if(!TCL_LISTA){					
			rts							;   return;
RELEASED:	bclr Banderas, $03			; }else{ TCL_LEIDA,TCL_LISTA = false;	(4)
...
PUSHED:		ldaa Tecla_IN				; 
			...
			bclr Banderas, $03			;	TCL_LEIDA,TCL_LISTA = false;
			rts							; }else{							(3)
...
PRESSED:	
			pshb					; save i2 index
			ldab Patron 			; index_tecla= (i1-1)*3+i2+1; 
			decb
			ldaa #3
			mul
			addb 0,SP
...
ENTER:		brclr Cont_TCL,$FF,FA_R			; void enter(){ if(Cont_TCL == 0){return;};		
			...
			bset Banderas,$04				;   ARRAY_OK = true;
			rts								;   return;
											; }		
